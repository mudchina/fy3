...
void create()
{
    set("short", "桃花仙镜");
    set("long", @LONG
眼面是波涛翻滚的大海(sea)。浪花滚滚迩来，气势洪博。
摇望远方一点孤舟时隐时现。银色的沙滩，兰色的海天。海鸟
的鸣叫，有如仙境一般。海边有个简易码头，可是没有看有什
么人，南边有一条崎岖小路。饿了怎么办呢？急的直想叫嚷！

LONG );
    set("exits", ([
        "south" : __DIR__"xiaoyucun.c",
    ]));
    set("item_desc", ([
        "sea" : "近岸处有一叶小舟，也许喊(yell)一声船家就能听见。\n",
    ]));
    set("no_clean_up", 0);
...
void arrive()
{
    ...
    if( room = find_object(__DIR__"duchuan") )
    {
        room->set("exits/out", __DIR__"102.c");
