...
set("xxy/rank","进士");
set("inquiry",([
"here":"这里是举人考场，想考试的话，最好看看墙上的告示(gaoshi).\n",
"考试":"看墙上的告示吧。\n",
]));
setup();
}
...
int do_exam()
{
 ...
 if (ob->query_skill("literate",1)<120) return notify_fail("主考官摇头道：等你读书写字进步些再来吧.\n");
...
if (level)  {
       ...
       if (level>3)
       return notify_fail("主考官惊讶道：你已是"+rank+"，还考什么进士呢?\n");
       if (level<3)
       return notify_fail("主考官笑道：一步步来，等考上举人再说。\n");
        }
 if (now_time()!=1) return notify_fail("主考官摇头道：每月初一开考，你来晚了。\n");
if (now_time()==ob->query("xxy/last_check")) return notify_fail("主考官瞄了你一眼道：下个月再来吧!\n");
...
message_vision("$N沉思良久，道：此题答案可是["+arg+"]?\n",ob);
   if (check_time<60&&arg==ob->query_temp("xxy/answer")) {
   command("nod");
   command("chat 恭喜"+ob->query("name")+"顺利考取进士。\n");
   ob->set("xxy/rank","进士");
   ob->set("xxy/lvl",4);
   ob->delete_temp("xxy");
