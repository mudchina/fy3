...
string short()
{
	...
	if( unread )
		return sprintf("%s "HIY"["HIC" %d "NOR"张留言，"HIC"%d "NOR"张未读 "HIY"]"NOR, ::short(), sizeof(notes), unread);
	else
		return sprintf("%s "HIY"["HIC" %d"NOR" 张留言 "HIY"]"NOR, ::short(), sizeof(notes));
}
...
int do_post(string arg)
{
	...
	object me=this_player();
        ...
        if( (int)query("read_only") && !wizardp(me))
		return notify_fail("你无权在此板上发表文章。\n");
...
"[%d] %-34s %s<%s>(%s)\n----------------------------------------------------------------------\n",
		num + 1, notes[num]["title"],notes[num]["author"],notes[num]["id"],ctime(notes[num]["time"])[0..9])
		+ notes[num]["msg"] );
