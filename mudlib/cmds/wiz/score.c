...
int main(object me, string arg)
{
	object ob,lp;
	mapping my;
	string line, str, skill_type , marry;
	string banghui;
	object weapon;
	...
	if(!arg)
		...
		return notify_fail("只有巫师或同帮会的人才能察看别人的状态。\n");
	...
	if( wizardp(me) || (int)ob->query("age") >= 16 ) {
             if(!wizardp(me)){
                line += sprintf(
                        " 膂力：[%s]  胆量：[%s]  悟性：[%s]  灵性：[%s]\n"
                        " 定力：[%s]  魅力：[%s]  根骨：[%s]  福缘：[%s]\n\n",
		...
		line += sprintf(
			" 膂力(str)：[%s]  胆量(cor)：[%s]  悟性(int)：[%s]  灵性(spi)：[%s]\n"
			" 定力(cps)：[%s]  魅力(per)：[%s]  根骨(con)：[%s]  福缘(kar)：[%s]\n\n",
			display_attr(my["str"], ob->query_str()),
			display_attr(my["cor"], ob->query_cor()),
			display_attr(my["int"], ob->query_int()),
			display_attr(my["spi"], ob->query_spi()),
		...
		marry ="你的伴侣是: "+ob->query("marry/name");
             }
	if( mapp(my["family"]) ) {
		if( my["family"]["master_name"] )
			line = sprintf("%s 你的师父是%s  %s  \n\n",
				line, my["family"]["master_name"],marry );
	}
...
//		(1 + (int)ob->query("learned_points")) * 100 / (1 + (int)ob->query("potential")) );
	...
	line += sprintf("江湖名望："HIM "%d      " NOR, ob->query("mingwang"));
	line += sprintf("杀气：" RED "%d\n\n"NOR, ob->query("bellicosity"));
...
为了避免玩家为了「天赋」的数值而反覆创造人物，因此限定只有在达到16岁
以上的玩家或巫师才看得到天赋状况。
