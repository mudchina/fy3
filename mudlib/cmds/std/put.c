...
//测试目标物件是否需要打开 add by xxy.
   if (dest->query("vacuous/status")) return notify_fail(dest->name()+"还没打开呢!\n");
       if(dest->query_temp("block_msg/all"))
                 return notify_fail("你现在不能这麽做。\n");
...
int do_put(object me, object obj, object dest)
{
	...
	if( obj->query("no_drop"))
		return notify_fail("这个东西不能丢弃！\n");
	if( obj==dest)
		return notify_fail("什么？\n");

	inv=all_inventory(dest);
	if(sizeof(inv)>=20)	return notify_fail(dest->query("name")+"中东西太多，装不下了！\n");
	if( obj->move(dest) ) {
