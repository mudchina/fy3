...
int main(object me, string arg)
{
	...
	if( (int)environment(me)->query("hotel") && !wizardp(me))
		return notify_fail("店小二探头进来，瞪了你一眼！\n");
	...
	if(sizeof(guards))	
		return notify_fail(env->query("name")+"正被"+
		guards[0]->query("name")+"看守着，你无法从中拿取东西。\n");

//check 目标物件是否要打开,add by xxy.
if (env->query("vacuous/status")) return notify_fail(env->name()+"还没打开呢!\n");
	...
	if(sizeof(guards))
		return notify_fail(obj->query("name")+"正被"+
		guards[0]->query("name")+"看守着！\n");
