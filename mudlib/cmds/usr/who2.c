...
mixed main(object me, string arg, int remote)
{
	...
	if( opt_long ) {
		if( (int)me->query("sen") <30 && ! wizardp(me))
			return notify_fail("你的精神太差了，没有办法得知其他玩家的详细资料。\n");
		if(! wizardp(me))	me->receive_damage("sen",30);
	}
...
int sort_user(object ob1, object ob2)
{
	...
	if( (string)ob1->query("banghui")!=(string)ob2->query("banghui"))
		return -1;
	if( (string)ob1->query("banghui")==(string)ob2->query("banghui"))
		return 1;
