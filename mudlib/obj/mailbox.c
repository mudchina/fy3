...
void create()
{
	set_name("信箱", ({"xinxiang", "mailbox", "box","xin xiang", "xiang" }) );
	set("long",
		"这是你的私人信箱，你可用以下命令来处理你的信：\n\n"
		"mail <某人>               寄信给别人。\n"
		"forward <信件编号> <某人> 将信转寄给别人。\n"
		"from                      查看信箱中的信件。\n"
		"read <信件编号>           阅读一封信件。\n"
		"discard <信件编号>        丢弃一封信件。\n"
	);
...
void init()
{
	...
	if( this_player() && environment()==this_player() ) {
		...
		set_name( this_player()->name(1) + "的信箱", ({ "xinxiang","mailbox", 
"box" }) );
...
void send_mail(string rcvr, mapping mail)
{
	...
	if( !ob ) {
        ...
        if(ppl) write(ob->query("name")+"目前正在泥巴世界中。\n");     
        write("你写了封信给"+ob->query("name")+"。\n");

	// Get the mailbox or make a new one if not found.
	ppl = find_player(rcvr);
	if(ppl) mbx = ppl->query_temp("mbox_ob");
        if(ppl) tell_object(ppl, "\n你突然觉得应该到驿站去看看 ...\n\n");
	if( !mbx ) {
		mbx = new(MAILBOX_OB);
		mbx->set_owner(rcvr);
	           }
