...
void on_board()
{
	...
	for (j=0; j<sizeof(all); j++) 
	{
      ...
      room = load_object(NOWDIR+"duchuan2");
	...
	if(i==1)
	{
		...
		if( clonep(this_object()) ) call_out("arrive", 20, 0);
	}
	else
	{
		this_object()->move(NOWDIR+"dukou_jx");
	}	
}
...
void arrive()
{
	...
	for (i=0; i<sizeof(all); i++) 
	{
		if( all[i]==this_object() ||!all[i]->is_character()) continue;
		{
			...
			all[i]->move(NOWDIR+"dukou_thd");
			all[i]->set_temp("win_shagu",0); 
		}
	}
...
int do_ask(string arg)
{
	...
	me=this_player();	
      ...
      if((arg!="chuan fu about in")&&(arg!="chuan fu about 桃花岛"))
	{
	...
	load_object(NOWDIR+"dukou_jx")->set("exits/enter", NOWDIR+"duchuan2");
	this_object()->move(NOWDIR+"duchuan2");
	load_object(NOWDIR+"duchuan2")->set("exits/out", NOWDIR+"dukou_jx");

	if( clonep(this_object()) ) call_out("on_board", 10, 0);
