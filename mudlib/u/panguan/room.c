// roommaker.c  wade 版 1.x
// 连结方向时可以用缩写
// 造房间时可以自动连接出口
//   并且会依序问问题, 巫师只要回答即可, 有点像问卷
//   如果要用预设值, 直接按 enter
//   如果想放弃编辑, 在输入的该行, 输入句点 "." 	<-- 就像在 post
//   如果有不备之处, 请随时 post
//
//   PS: 本 roommaker 不会因为 saveroom 指令造成盖掉大部份 function 的错误
//   PS: 本档欢迎拿去修改, 不过请勿流入玩家手中

#define RMK_VERSION "(v 1.4 11/30/1995)"

inherit ITEM;
inherit __DIR__"dird.c";
...
int get_no_magic (string rep)
{
  ...
  write ("请输入 light_up 的值 (0 预设为没灯光的房间) : ");
...
{
  if (rep == ".") { write ("你放弃本次编辑。\n"); return 1; }
  else if (rep == "") tmp["light_up"] = 0;
  else if (sscanf (rep, "%d", tmp["light_up"]) != 1) {
    write ("你的输入不是数字, 应该是 0 或 1\n");
    write ("请输入 light_up 的值 (0 预设为可施魔法) : ");
    input_to ( "get_light_up" );
    return 1;
  }
