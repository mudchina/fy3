...
void create ()
{
    set ("short", HIY"逍遥赌坊"NOR);
	set ("long", @LONG
大厅里人声鼎沸，不时传来栈阵阵或激动，或感慨，或无奈的声音
中央是一张足足可以容纳三十人的大圆桌，桌子边挤满了男女老少，由
于人多，空气十分的浑浊，似乎人们都不注意这些，每个人的眼睛都死
死的盯着....再往里是贵宾厅，没有万贯家财最好还是在这里混混时间
吧！迎面的牌子(paizi)上写着赌坊的规则。
LONG);
...
　　局，赌注可取回。
	...
	set("exits", 
	([ //sizeof() == 2
		"east" : __DIR__"chunxilu3",
"north" : "/d/zhongzhou/yinghao",
	]));
...
int do_cancel()
{
	...
	if( me->query_temp("shaizi_cancel_count") > 5 )
		return notify_fail("俗话说：嫖情赌意，你娃一点信誉都无得唆！！\n"); 
	...
	{//if I am not zhuang-jia.
		if( me->query_temp("shaizi/bet_amount") <= 0 ) 
			return notify_fail("你娃又没下注，在这里起啥子哄？\n"); 
...
int do_retire()
{
	...
	if(game_status == 1 || game_status == 0 )
	{
		message_vision("$N站起来嚷道：今天啷个楞个霉，庄我可不坐了！说罢顺手将桌上的保证金揣在怀里。\n", me);
		zhuang_id = "no zhuang";
		game_status = 0;
...
{
	int max_limit = 10; //1000 gold.
	int min_limit = 1; //10 gold.
	int current_limit;
	...
	if(game_status == 1 || game_status == 0 )
	{
		...
		if(current_limit < min_limit)
			return notify_fail("你这穷鬼？？？还是别坐庄了。快滚！！！\n赌注上限不能少于"+chinese_number(min_limit)+"两黄金。\n");
