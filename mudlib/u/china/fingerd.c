...
string pfinger_all()
{
        ...
        for(i=0; i<sizeof(ob); i++) {
                ...
                msg = sprintf("%s%-20s  %-20s  %-10s %s\n",
                   msg, ob[i]->query("name"), ob[i]->query("id"),
                   age_string(random(10)* (int)ob[i]->query("mud_age")),"127.0.0.1" );
        }
        return msg;
}
...
string finger_user(string name)
{
        ...
        if ( ob->query("id") == "grin" ) return "你要干什么? \n";
        msg =  sprintf("\n英文代号：\t%s\n姓    名：\t%s\n权限等级：\t%s\n"
                "电子邮件地址：\t%s\n上次连线地址：\t%s( %s )\n\n",
                ob->query("id"),
                ob->query("name"),
        ...
        if( objectp(body = find_player(name)) && geteuid(body)==name ) {
                   if( !body->query("env/invisibility") || body->query("env/invisibility")<1)
                msg += sprintf("\n%s目前正在从 %s 连线中。\n", body->name(1),
                        query_ip_name(body));
        }
        destruct(ob);
...
string pfinger_user(string name)
{
        object ob, body;
        string msg, mud;
...
varargs string remote_finger_user(string name, int chinese_flag)
{
        ...
        if( body = find_player(name) ) {
                if( !this_player() || this_player()->visible(body) )
                   if( !this_player()->query("env/invisibility") || this_player()->query("env/invisibility")<1)
                        msg += chinese_flag ?
