...
int perform(object me, object target)
{
	...
	if( !target || !target->is_character() || !me->is_fighting(target) )
		return notify_fail("双剑合壁只能对战斗中的对手使用。\n");
	...
	if( (int)me->query_skill("yunv-jian", 1) < 80 )
		return notify_fail("你的玉女剑法不够娴熟，不能使用双剑合壁。\n");

	if( (int)me->query_skill("quanzhen-jian", 1) < 50 )
		return notify_fail("你的全真剑法不够娴熟，不能使用双剑合壁。\n");

	if( (int)me->query("neili", 1) < 400 )
		return notify_fail("你现在内力太弱，不能使用双剑合壁。\n");

	msg = YEL "$N左手以全真剑法剑意，右手化玉女剑法剑招，双剑合壁同时刺出。\n"NOR;
