...
if (now_time()==ob->query("xxy/last_check")) return notify_fail("主考官瞄了你一眼道：明天再来吧!\n");
 ...
 return notify_fail("主考官指着你道：问题还没回答吧!/n");
 give_quest(ob);
return 1;
}
int give_quest(object ob)
...
if (random(2)==1) {
 ...
 if (!ob->query_temp("xxy/count")) ob->set_temp("xxy/count",1);
 else ob->add_temp("xxy/count",1); 
...
if (arg==ob->query_temp("xxy/answer")) {
   check_time=ob->query_temp("xxy/time")-time();
   if (check_time>300) {
   ...
   command("nod");
   if (ob->query_temp("xxy/count")<2) give_quest(ob);
   else  {
