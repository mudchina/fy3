...
int practice_skill(object me)
{
		...
		return notify_fail("你的体力太差了，无法练习慕容身法。\n");
		me->receive_damage("kee",30);
        return 1;
}
string perform_action_file(string action)
{
		return CLASS_D("xiake")+"/murongshenfa/"+action;
}
void skill_improved(object me)
{
		if(me->query_skill("murongshenfa",1)==40)
		tell_object(me,HIC"你领悟了「吹浪三叠式」的身法！(chuilang)\n"NOR);
}
